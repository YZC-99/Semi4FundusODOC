...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--config', type=str, default='Drishti-GS/cropped_sup256x256/my_segformer/v7-ii-1-6-v1/loss/contrast/b2_CE_FC_Contrast_Loss')
    ...
    for fc in np.arange(1,11,1):
        for contrast in np.arange(1,11,1):
            # org_config = args.config
            args.config = args.config.replace("FC","{}e-1-FC".format(fc))
            args.config = args.config.replace("Contrast","{}e-1-Contrast".format(contrast))
            config.MODEL.FC_loss = fc * 0.1
            config.MODEL.ContrastCrossPixelCorrect_loss = contrast * 0.1
