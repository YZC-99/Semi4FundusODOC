...
class Base(pl.LightningModule):
    def __init__(self,
        ...
        if model == 'deeplabv3plus':
            ...
            self.model = DeepLabV3Plus(self.backbone,self.num_classes,Isdysample=cfg.MODEL.Isdysample)
        if model == 'deeplabv2':
            self.backbone = backbone
        ...
        if cfg.MODEL.ContrastCrossPixelCorrectCBL_loss is not None:
            self.ContrastCrossPixelCorrectCBL_loss = ContrastCrossPixelCorrectCBL(self.num_classes, cfg.MODEL.ContrastCrossPixelCorrectCBL_loss)
    ...
    def compute_loss(self,output,batch):
        ...
        if self.cfg.MODEL.ContrastPixelCorrectCBL_loss:
            loss = loss + self.ContrastPixelCorrectCBL_loss(output, y, self.model.classifier.weight,
                                                      self.model.classifier.bias)
        if self.cfg.MODEL.ContrastCrossPixelCorrectCBL_loss:
            loss = loss + self.ContrastCrossPixelCorrectCBL_loss(output, y, self.model.classifier.weight,
                                                            self.model.classifier.bias)
