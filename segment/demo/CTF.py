...
class ASPPModule(nn.Module):
    def __init__(self, in_channels, atrous_rates,down_ratio=8):
...
if __name__ == '__main__':
    ...
    # lateral_connections_c1 = ASPPModule(256,(12,24,36),down_ratio=1)
    ...
    cft_net = cft(256,num_classes)

    # 初始化一个criss attention
    criss_attention = CrissCrossAttention(256)
    # 现在需要解决的问题是:要将c4_out 与 c3进行cross attention
    '''
    根据图可以知道，进行cross attention后是要与 c3进行相加的，因此cross attention的形状要与c3一致
    c3 = c3.view(B,-1,256)
    '''
    cft_net_out = cft_net(c4,c3)#(B,L*C,H,W)

    print(cft_net_out.shape)
